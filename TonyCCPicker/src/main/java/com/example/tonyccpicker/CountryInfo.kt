...
data  class CountryInfo(var name : String, var nameCode : String, var phoneCode : String) : Comparable<CountryInfo> {
    private var flagId : Int? = null
    ...
    companion object{
        private var countries : MutableList<CountryInfo>? = null

        fun getAllCountries(context: Context, language : String) : MutableList<CountryInfo>{
            ...
            try {
                ...
                val ins = context.resources.openRawResource(
                    context.resources
                        .getIdentifier(
                            "ccp_" + language.toLowerCase(Locale.ROOT),
                    ...
                    when (event) {
                        ...
                        XmlPullParser.END_TAG -> if (name == "country") {
                            val countryInfo = CountryInfo(
                                ...
                                xmlPullParser.getAttributeValue(null, "name_code").toUpperCase(),
        ...
        fun getFilteredCountries(context : Context, language : String, keyword : String) : MutableList<CountryInfo>{
            if(countries==null){
                countries = getAllCountries(context, language)
            }

            var filteredCountries = mutableListOf<CountryInfo>()

            for(country in countries!!){
                if(keyword in country.name){
        ...
        fun getFlagMasterResID(country: CountryInfo): Int {
